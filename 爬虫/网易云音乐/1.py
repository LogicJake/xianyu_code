...
def get_comments(name, id, page):
    """
    获取评论信息
    """
    url = f'http://music.163.com/api/v1/resource/comments/R_SO_4_{id}?limit=10&offset=0' + str(
        page)
    ...
    for item in items:
        ...
        print(user_name, user_id, user_age, user_gender, user_city,
              user_introduce, comment, comment_id, praise, date)

        with open(f'{name}_comments.csv', 'a', encoding='utf-8-sig') as f:
            f.write(user_name + ',' + user_id + ',' + user_age + ',' +
                    user_gender + ',' + user_city + ',' + user_introduce +
...
def get_user(user_id):
    ...
    if js['code'] == 200:
        ...
        if int(js['profile']['birthday']) < 0:
            ...
            data['age'] = (2018 - 1970) - (int(js['profile']['birthday']) //
                                           (1000 * 365 * 24 * 3600))
        if int(data['age']) < 0:
            data['age'] = 0
        # 城市
...
def main(name, id):
    ...
    for i in range(0, 40, 20):
        print('\n---------------第 ' + str(i // 20 + 1) + ' 页---------------')
        get_comments(name, id, i)


if __name__ == '__main__':
    df = pd.read_csv('热歌榜 - 排行榜 - 网易云音乐.csv', sep='|')

    for name, id in df[['歌曲名', '歌曲id']].values:
        id = id[3:]
        main(name, id)
